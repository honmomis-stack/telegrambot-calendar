...
// --- BYOK Gemini API Key ---
export async function saveGeminiKey(telegramId, apiKey) {
  const encKey = apiKey ? encrypt(apiKey) : null;
  const { error } = await db()
    .from('users')
    .update({ gemini_api_key: encKey })
    .eq('telegram_id', telegramId);
  if (error) throw error;
}

export function getGeminiKey(user) {
  if (!user || !user.gemini_api_key) return null;
  try {
...
// --- Group & Finance Features ---

export async function ensureGroup(chatId, title) {
  const { error } = await db()
    .from('groups')
    .upsert({ chat_id: chatId, title: title || 'Group' }, { onConflict: 'chat_id' });
  if (error) console.error('ensureGroup error:', error.message);
}

export async function saveTransaction(chatId, userId, amount, currency, trxType, desc, trxDate) {
  const { error } = await db()
    .from('transactions')
    .insert({
      chat_id: chatId,
...
export async function getTransactions(chatId, startDate, endDate) {
  const { data, error } = await db()
    .from('transactions')
    .select('*')
    .eq('chat_id', chatId)
...
export async function saveGroupMessage(chatId, userId, username, text) {
  const { error } = await db()
    .from('group_messages')
    .insert({
      chat_id: chatId,
  ...
  if (error) console.error('saveGroupMessage error:', error.message);
}

export async function getRecentGroupMessages(chatId, limit = 50) {
  const { data, error } = await db()
    .from('group_messages')
    .select('username, text_content, created_at')
    .eq('chat_id', chatId)
