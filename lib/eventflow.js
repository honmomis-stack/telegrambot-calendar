...
// ---- AI Event Flow (JSON ពី Gemini) ----
export async function startAiEventFlow(chatId, userId, isPrivate, data) {
  if (!isPrivate) {
    return sendMessage(chatId, '🔒 សូមបង្កើតព្រឹត្តិការណ៍ក្នុង chat ឯកជន (DM) ជាមួយ bot។');
  }
  ...
  await saveDraft(userId, {
    step: 'reminder',
    summary: data.summary || 'ព្រឹត្តិការណ៍ (AI)',
    event_date: data.date,
    event_time: data.time || '00:00',
    reminder_kind: null,
  });
  
  const msg = `✨ <b>AI បានរៀបចំព្រឹត្តិការណ៍ជូនអ្នក៖</b>

📌 <b>ឈ្មោះ៖</b> ${esc(data.summary)}
📅 <b>ថ្ងៃទី៖</b> ${data.date}
⏰ <b>ម៉ោង៖</b> ${data.time || 'ពេញមួយថ្ងៃ'}

🔔 តើអ្នកចង់ឲ្យរំលឹកបែបណា?`;
  
  return sendMessage(chatId, msg, reminderKeyboard());
}
