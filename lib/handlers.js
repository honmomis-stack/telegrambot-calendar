...
import { sendMessage, sendPhoto, answerCallbackQuery, urlButton, inlineButtons, webAppButton, esc, sendChatAction } from './telegram.js';
...
function mainKeyboard() {
  return {
    reply_markup: {
      keyboard: [
        [{ text: '📅 ប្រតិទិនខ្មែរ', web_app: { url: 'https://telegrambot-calendar.vercel.app/' } }],
        [{ text: BTN.NEW }],
...
async function handleMessage(message) {
  ...
  if (!isPrivate) {
    await ensureGroup(chatId, message.chat.title);
    if (rawText) {
      await saveGroupMessage(chatId, userId, message.from.first_name || 'User', rawText);
    }
  }
  ...
  switch (command) {
    ...
    case '/setgemini':
      if (!args[0]) return sendMessage(chatId, `⚠️ សូមបញ្ចូល Key របស់អ្នក។\nទម្រង់៖ <code>/setgemini AIzaSy...</code>`);
      try {
        await saveGeminiKey(userId, args[0]);
        return sendMessage(chatId, '✅ Gemini API Key របស់អ្នកត្រូវបានរក្សាទុកដោយសុវត្ថិភាព (Encrypted)។ អ្នកអាចសាកល្បងនិយាយជាមួយ Bot បានឥឡូវនេះ!');
      } catch(e) {
        return sendMessage(chatId, '❌ បរាជ័យក្នុងការរក្សាទុក Key។ សូមសាកល្បងម្ដងទៀត។');
      }

    case '/removegemini':
      try {
        await saveGeminiKey(userId, null);
        return sendMessage(chatId, '🗑 Gemini API Key របស់អ្នកត្រូវបានលុបចេញពីប្រព័ន្ធដោយជោគជ័យ។');
      } catch(e) {
        return sendMessage(chatId, '❌ បរាជ័យក្នុងការលុប Key។');
      }

    case '/mygemini':
      const u = await getUser(userId);
      const k = getGeminiKey(u);
      if (k) return sendMessage(chatId, '🔑 អ្នកបានបញ្ចូល Key ផ្ទាល់ខ្លួនរួចរាល់ហើយ។ (វានៅតែមានសុវត្ថិភាពក្នុងប្រព័ន្ធអ៊ិនគ្រីប)');
      return sendMessage(chatId, '❌ អ្នកមិនទាន់បានបញ្ចូល Key ផ្ទាល់ខ្លួនទេ។');

    case '/report':
      if (isPrivate) return sendMessage(chatId, '⚠️ សូមប្រើ /report នៅក្នុង Group របាយការណ៍។');
      const todayDate = new Date().toISOString().slice(0,10);
      const startOfMonth = todayDate.slice(0,8) + '01';
      const trxs = await getTransactions(chatId, startOfMonth, todayDate);
      if (trxs.length === 0) return sendMessage(chatId, '📊 មិនមានរបាយការណ៍ហិរញ្ញវត្ថុក្នុងខែនេះទេ។');
      ...
      let repMsg = `📊 <b>របាយការណ៍ហិរញ្ញវត្ថុប្រចាំខែ</b>\n\n`;
      repMsg += `💵 <b>ចំណូលសរុប៖</b> ${totalInc.toFixed(2)}$\n`;
      repMsg += `💸 <b>ចំណាយសរុប៖</b> ${totalExp.toFixed(2)}$\n`;
      repMsg += `⚖️ <b>សល់/ខ្វះ៖</b> ${(totalInc - totalExp).toFixed(2)}$\n`;
      return sendMessage(chatId, repMsg);

    case '/summary':
      if (isPrivate) return sendMessage(chatId, '⚠️ សូមប្រើ /summary នៅក្នុង Group។');
      const gMsgs = await getRecentGroupMessages(chatId, 50);
      if (gMsgs.length === 0) return sendMessage(chatId, 'មិនទាន់មានសារអ្វីក្នុង Group នេះដើម្បីសង្ខេបទេ។');
      const allText = gMsgs.map(m => `${m.username}: ${m.text_content}`).join('\n');
      const tPrompt = `សូមសង្ខេបការសន្ទនាខាងក្រោមឲ្យបានខ្លីនិងងាយយល់បំផុត៖\n\n${allText}`;
      const sReply = await generateResponse(tPrompt, 'Asia/Phnom_Penh', null, 'private');
      return sendMessage(chatId, `📝 <b>សង្ខេបការសន្ទនាថ្មីៗ៖</b>\n\n${sReply}`);
    ...
    default:
      ...
      // អត្ថបទធម្មតា → ហៅ Gemini AI
      await sendChatAction(chatId, 'typing');
      const userRec = await getUser(userId);
      const tz = userRec?.timezone || config.defaultTimezone;
      const chatType = isPrivate ? 'private' : 'group';
      const aiReply = await generateResponse(text, tz, getGeminiKey(userRec), chatType);
      if (aiReply) {
        try {
          const firstBrace = aiReply.indexOf('{');
          const lastBrace = aiReply.lastIndexOf('}');
          if (firstBrace >= 0 && lastBrace >= 0 && lastBrace > firstBrace) {
            const jsonStr = aiReply.substring(firstBrace, lastBrace + 1);
            const parsed = JSON.parse(jsonStr);
            if (parsed.intent === 'create_event' || (parsed.summary && parsed.date)) {
              return await startAiEventFlow(chatId, userId, isPrivate, parsed);
            } else if (parsed.intent === 'finance') {
              if (isPrivate) return sendMessage(chatId, '⚠️ មុខងារកត់ត្រាលុយកាក់ ប្រើបានតែនៅក្នុង Group ប៉ុណ្ណោះ។');
              const tDate = new Date().toISOString().slice(0, 10);
              await saveTransaction(chatId, userId, parsed.amount, parsed.currency || 'USD', parsed.type, parsed.desc, tDate);
              const symbol = parsed.currency || '$';
              return sendMessage(chatId, `✅ កត់ត្រា${parsed.type === 'income' ? 'ចំណូល' : 'ចំណាយ'} <b>${parsed.amount} ${symbol}</b> រួចរាល់! (${parsed.desc})`);
            } else if (parsed.intent === 'summary') {
              if (isPrivate) return sendMessage(chatId, '⚠️ មុខងារសង្ខេប ប្រើបានតែនៅក្នុង Group ប៉ុណ្ណោះ។');
              const msgs = await getRecentGroupMessages(chatId, 50);
              if (msgs.length === 0) return sendMessage(chatId, 'មិនទាន់មានសារអ្វីក្នុង Group នេះដើម្បីសង្ខេបទេ។');
              
              const combinedText = msgs.map(m => `${m.username}: ${m.text_content}`).join('\n');
              const sumPrompt = `សូមសង្ខេបការសន្ទនាខាងក្រោមឲ្យបានខ្លីនិងងាយយល់បំផុត៖\n\n${combinedText}`;
              const summaryReply = await generateResponse(sumPrompt, tz, getGeminiKey(userRec), 'private');
              return sendMessage(chatId, `📝 <b>សង្ខេបការសន្ទនាថ្មីៗ៖</b>\n\n${summaryReply}`);
            }
          }
      ...
      return sendMessage(chatId, aiReply);
  }
}
